...
fit = 
  ...
  mutate(
    ...
    room_type = fct_infreq(room_type) #Manhattan
  ) |> 
...
nyc_airbnb |> 
  nest(df = -borough) |> 
  mutate(
    models = map(df, \(df) lm(price ~ stars + room_type, data = df)), #don't need to write function in other place
    results = map(models, broom::tidy)
  ) |> 
  select(borough, results) |> 
  unnest(results) |> 
...
fit_logistic |> 
  broom::tidy() |> 
  mutate(OR = exp(estimate)) |> #exponatiate the result
